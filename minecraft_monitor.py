...
# --- RCON CONFIGURATION (MUST MATCH YOUR MINECRAFT SERVER SETTINGS) ---
# 3. Server IP/Hostname (REQUIRED)
RCON_HOST = "127.0.0.1"
# 4. Server RCON Port (Default Minecraft RCON port is 25575)
RCON_PORT = 25575
...
async def _get_current_players() -> set:
    """Gets the list of currently logged-in players using the 'list' command."""
    # Minecraft 'list' command response looks like:
    # "There are 2 of a max of 20 players online: PlayerA, PlayerB"
    response = await _send_rcon_command("list")
    
    # Check for RCON error first
    if response.startswith("ERROR:"):
        return set(), response

    players = set()
    # Find the players list part of the string after the colon
    if ':' in response:
        player_list_str = response.split(':', 1)[1].strip()
        # Split by comma and clean up spaces, ignoring empty strings
...
async def help_command(ctx):
    ...
    **!mine-players**
    > Lists all currently logged-in players (names only).

    **!mine-say <message>**
    > Sends a message to the in-game chat, prefixed by `[Discord Admin]`.
    > Example: `!mine-say Welcome to the server!`

    **!mine-save**
    > Forces the server to immediately save the world state (`save-all`).

    **!mine-stop**
    > Forces the server to shut down gracefully (`stop`).
...
async def status_command(ctx):
    ...
    if not await _connect_rcon():
        ...
        # Try to extract player count from the 'list' response
        player_count_match = 0
        try:
            # Example response: "There are 2 of a max of 20 players online: PlayerA, PlayerB"
            count_match = raw_response.split(' of a max of ')[0].split('are ')[-1].strip()
            player_count_match = int(count_match)
        except Exception:
            player_count_match = len(players)
            
        embed = Embed(
            title=f"🟢 {GAME_NAME} Server Status",
            description=f"**{status_msg}**\n\nPlayers Online: **{player_count_match}**",
            color=status_color
        )
...
async def players_command(ctx):
    ...
    embed.add_field(name="Names", value=player_list_str, inline=False)
    await ctx.send(embed=embed)
...
async def say_command(ctx, *, message: str):
    ...
    # Minecraft RCON command to send a message to chat
    full_message = f"[Discord Admin] {message}"
    command = f"say {full_message}"
    response = await _send_rcon_command(command)

    if "ERROR:" in response:
        await ctx.send(f"❌ **Say Failed!** {response}")
    else:
        embed = Embed(
            title="📣 Message Sent In-Game",
...
async def save_command(ctx):
    ...
    if "ERROR:" in response:
        ...
        # Minecraft save-all usually returns 'Saving...' then 'Saved the world'
        embed = Embed(
            title="💾 World Saved",
...
async def stop_command(ctx):
    """Forces the server to shut down gracefully."""
    command = "stop"
    response = await _send_rcon_command(command)

    if "ERROR:" in response:
        await ctx.send(f"❌ **Stop Failed!** {response}")
    else:
        # Minecraft 'stop' usually initiates the shutdown process
        embed = Embed(
            title="🛑 Server Stop Initiated",
            description="Server will begin graceful shutdown. You must restart the server manually.",
            color=Colour.red()
        )
        await ctx.send(embed=embed)
...
@commands.has_permissions(administrator=True)
async def kick_command(ctx, player_name: str):
    """Kicks a player using their exact in-game name."""
    command = f"kick {player_name}"
    response = await _send_rcon_command(command)

    # Minecraft RCON response is usually an empty string on success
    if "ERROR:" in response:
        await ctx.send(f"❌ **Kick Failed!** Make sure the player name (`{player_name}`) is correct and the player is online. Response: {response}")
    else:
        embed = Embed(
            title="👟 Player Kicked",
...
@commands.has_permissions(administrator=True)
async def ban_command(ctx, player_name: str):
    """Bans a player using their exact in-game name."""
    ...
    # Minecraft RCON response is usually an empty string on success
    if "ERROR:" in response:
        await ctx.send(f"❌ **Ban Failed!** Make sure the player name (`{player_name}`) is correct. Response: {response}")
    else:
        embed = Embed(
            title="🔨 Player Banned",
